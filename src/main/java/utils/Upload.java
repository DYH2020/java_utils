				...
				if (part.isFile()) { // 判断是否是文件
					...
					if (fileName != null && fileName.length() > 0) {
						...
						if (fileExtName.equalsIgnoreCase("jpeg")
								...
								// 在这里再加一个视频的格式，不知道会是什么样的情况
								|| fileExtName.equalsIgnoreCase("webm")) {
							Random random = new Random();
							int n = random.nextInt(10000);
							String newFileName = new Date().getTime() + n + "."
							...
							String newFileName = "goods" + "." + fileExtName;// 重新改文件名
							// 文件名+扩展名
							String newPath = path + "/" + newFileName; // 文件处理文件上传的路径
							File newFile = new File(newPath);
							filepart.writeTo(newFile); // 将文件真正写入到对应的文件夹中
						...
						} else if (fileExtName.equalsIgnoreCase("apk")) {
							path = request.getSession().getServletContext()
									.getRealPath("/appdownload/android");
							File newFile = new File(path);
							filepart.writeTo(newFile); // 将文件真正写入到对应的文件夹中
							map.put(filepart.getName(), filepart.getFileName());// 把文件信息保存到map中
							
						} else if (fileExtName.equalsIgnoreCase("ipa")) {
							path = request.getSession().getServletContext()
									.getRealPath("/appdownload/ios");
							File newFile = new File(path);
							filepart.writeTo(newFile); // 将文件真正写入到对应的文件夹中
							map.put(filepart.getName(), filepart.getFileName());// 把文件信息保存到map中
